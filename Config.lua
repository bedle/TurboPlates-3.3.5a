...
function ns:GetActiveLanguage()
    local savedLanguage = type(TurboPlatesDB) == "table" and NormalizeLanguage(TurboPlatesDB.language)
...
ns.L = {
    ...
    DistanceScaling = "Scale With Distance",
    DistanceYOffset = "Auto Distance Y Offset",
    DistanceYOffsetTip = "Gradually adjusts distant nameplates vertically to keep them easier to see. Nearby nameplates keep their normal position. Use the slider below to fine-tune the effect.",
    DistanceYOffsetMax = "Distance Y Offset",
    DistanceYOffsetMaxTip = "Adjusts how much distant nameplates move vertically. Positive values move them higher; negative values move them lower.",
  ...
  -- Castbar Highlight Spells
    ...
    ClasslessServerUnavailable = "not available for Classless servers",
    ...
    FriendlyNameColor = "Friendly Name",
    TappedColor = "Tapped",
    SecureColor = "Secure",
    TransColor = "Losing",
    InsecureColor = "Lost",
    ...
    RuneHeader = "Death Knight Runes",
    ShowRunes = "Enable Runes",
    RunesOnPersonalBar = "Show on Personal Bar",
    RuneStyle = "Style",
    RuneStyleSquare = "Square",
    RuneStyleRounded = "Rounded",
    RuneSize = "Size",
    RuneX = "Nameplate X",
    RuneY = "Nameplate Y",
    RunePersonalX = "Personal Bar X",
    RunePersonalY = "Personal Bar Y",
...
local function MergeDefaultSchema(dst, defaults, fillMissing)
    if type(dst) ~= "table" or type(defaults) ~= "table" then return end
    for key, defaultValue in pairs(defaults) do
        local value = dst[key]
        if value == nil then
            if fillMissing then dst[key] = DeepCopy(defaultValue) end
        ...
        elseif type(defaultValue) == "number" then
            if not IsFiniteNumber(value) then dst[key] = defaultValue end
...
local function RepairColorTable(value, defaultValue)
    if type(value) ~= "table" then return DeepCopy(defaultValue) end
    local r = Clamp01(value.r)
    local g = Clamp01(value.g)
    local b = Clamp01(value.b)
    if r == nil or g == nil or b == nil then
        return DeepCopy(defaultValue)
    end
    value.r, value.g, value.b = r, g, b
    if value.a ~= nil then
        value.a = Clamp01(value.a) or (defaultValue and defaultValue.a) or 1
    end
    return value
end

function ns:RepairSettingsStructure(settings, fillMissingTopLevel)
    ...
    for _, key in ipairs(colorKeys) do
        if settings[key] ~= nil and ns.defaults[key] then
            settings[key] = RepairColorTable(settings[key], ns.defaults[key])
        end
    end

    if type(settings.personal) == "table" and ns.defaults.personal then
        if settings.personal.healthColor ~= nil then
            settings.personal.healthColor = RepairColorTable(
                settings.personal.healthColor, ns.defaults.personal.healthColor)
        end
    end

    if type(settings.auras) == "table" and ns.defaults.auras then
        if settings.auras.debuffBorderColor ~= nil then
            settings.auras.debuffBorderColor = RepairColorTable(
                settings.auras.debuffBorderColor, ns.defaults.auras.debuffBorderColor)
        end
        if settings.auras.buffBorderColor ~= nil then
            settings.auras.buffBorderColor = RepairColorTable(
                settings.auras.buffBorderColor, ns.defaults.auras.buffBorderColor)
        end
    end

    if settings._distanceYOffsetAdjustmentV1 ~= 1 then
        if settings.distanceYOffsetMax == 12 then
            settings.distanceYOffsetMax = 0
        end
        settings._distanceYOffsetAdjustmentV1 = 1
    end
    if settings.distanceYOffsetMax ~= nil then
        local value = settings.distanceYOffsetMax
        if not IsFiniteNumber(value) then
            value = ns.defaults.distanceYOffsetMax or 0
        end
        value = math.floor(value + 0.5)
        if value < -15 then value = -15 elseif value > 50 then value = 50 end
    ...
    if type(settings.minimap) == "table" and ns.defaults.minimap then
        ...
        if type(settings.minimap.hide) ~= "boolean" then
            settings.minimap.hide = ns.defaults.minimap.hide
        end
    end
    ...
    if settings.highlightSpells ~= nil then
        ...
        if type(settings.highlightSpells) == "table" then
            for spellID, enabled in pairs(settings.highlightSpells) do
                local id = tonumber(spellID)
                if id and id > 0 and enabled then repaired[id] = true end
            end
        end
        settings.highlightSpells = repaired
    end

    if settings.learnedChannels ~= nil then
        local repaired = {}
        if type(settings.learnedChannels) == "table" then
            for spellName, duration in pairs(settings.learnedChannels) do
                if type(spellName) == "string" and IsFiniteNumber(duration)
                   and duration > 0 and duration <= 300 then
                    repaired[spellName] = duration
                end
            end
        end
        settings.learnedChannels = repaired
    end
...
function ns:LoadVariables()
    ...
    if type(TurboPlatesDB.friendlyNameColor) ~= "table" or not TurboPlatesDB.friendlyNameColor.r then
        TurboPlatesDB.friendlyNameColor = DeepCopy(ns.defaults.friendlyNameColor)
    end
...
heroClasslessGuard:SetScript("OnEvent", function()
    if TurboPlatesDB and ns.IsClasslessHeroPlayer and ns.IsClasslessHeroPlayer() then
