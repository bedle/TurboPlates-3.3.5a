...
local function ValidateAuraColorRules(rules)
    ...
    for _, rule in ipairs(rules) do
        if type(rule) == "table" then
            ...
            if spellID and spellID == spellID and spellID ~= math.huge and spellID ~= -math.huge
               and spellID > 0 and type(color) == "table" then
        ...
        for k, v in pairs(settings) do
            if k ~= "highlightSpells" then
                if k == "auras" and type(v) == "table" then
                    ...
                    if type(TurboPlatesDB.auras) ~= "table" then TurboPlatesDB.auras = {} end
                    for ak, av in pairs(v) do
                        if ak ~= "whitelist" and ak ~= "blacklist" then
